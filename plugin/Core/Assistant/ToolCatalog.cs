    ...
    public static class ToolCatalog
    {
        ...
        private static readonly Dictionary<string, string> DisplayNamesRu =
            new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["trace_openings_from_cad"] = "двери/окна по DWG/CAD",
        ...
        private static List<ToolDef> BuildDefinitions()
        {
            ...
            return new List<ToolDef>
            {
                ...
                T("get_cad_link_geometry",
                    "Read DWG/CAD ImportInstance lines in mm. For exploded DWG set includeModelLines=true.",
                    P(
                        ("cadLinkName", S(), null),
                        ...
                        ("minLengthMm", N(), null),
                        ("limit", N(), null),
                        ("includeModelLines", B(), "also Model/Detail lines"))),
                T("trace_walls_from_cad",
                    "Trace walls from DWG/model lines: double-line centerlines, thickness from gap, auto-match wall type by mm. wallTypeId required fallback. Prefer dryRun first.",
                    P(
                        R("wallTypeId"),
                        ("wallTypeId", N(), "fallback Wall type id"),
                        ("cadLinkName", S(), null),
                        ("layerFilter", S(), "e.g. wall"),
                        ("bboxMm", O(), "clip region mm"),
                        ("toleranceMm", N(), "default 50"),
                        ("includeModelLines", S(), "true|false|auto"),
                        ("autoMatchWallTypes", B(), "match type by measured thickness"),
                        ("dryRun", B(), "preview axes + thicknessClusters"),
                        ("heightMm", N(), "default 3000"))),
                T("trace_openings_from_cad",
                    "Trace doors/windows from DWG onto existing host walls. Doors are read from the DWG swing arc, so position, width, swing side and hinge match the underlay. category=door|window|both. doorTypeId/windowTypeId required. Prefer dryRun first. Not for A-GLAZ-CURT (use walls).",
                    P(
                        ("category", S(), "door|window|both"),
                        ("doorTypeId", N(), "required for door|both"),
                        ("windowTypeId", N(), "required for window|both"),
                        ("cadLinkName", S(), null),
                        ("layerFilter", S(), "override layers"),
                        ("maxHostDistanceMm", N(), "default 250"),
                        ("sillHeightMm", N(), "window sill default 900"),
                        ("autoMatchTypesByWidth", B(), "match type by mm width"),
                        ("strictLocation", B(), "exact CAD point or fail; default true"),
                        ("dryRun", B(), "preview planned + hostWallId"),
                        ("toleranceMm", N(), "verify default 100"))),
                T("trace_columns_from_cad",
                    "Trace columns from DWG onto the plan: rectangular and round, real section and rotation. Use this for columns instead of trace_walls_from_cad. Prefer dryRun first.",
                    P(
                        ("columnTypeId", N(), "fallback column type id"),
                        ("category", S(), "structural|architectural"),
                        ("cadLinkName", S(), null),
                        ("layerFilter", S(), "override column layers"),
                        ("minSizeMm", N(), "default 150"),
                        ("maxSizeMm", N(), "default 1500"),
                        ("heightMm", N(), "default 3000"),
                        ("autoMatchTypesBySize", B(), "match type by section in name"),
